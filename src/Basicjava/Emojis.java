...
public class Emojis {
    ...
    public static GraphicsGroup createSmileyFace(double size) {
        ...
        group.add(mouth);
        group.add(createEye(size,false));
        group.add(createEye(size,true));

        return group;
    }
    public static Ellipse createEye(double size, boolean right) {
        Ellipse eye;
        if(!right) {
             eye = new Ellipse(size * 0.3, size * 0.2, size * 0.1, size * 0.1);
        }
        else {
             eye = new Ellipse(size*.6,size*.4,size*.1,size*.1);
        }
